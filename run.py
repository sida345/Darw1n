...
def main_callback(service_provider: LockdownClient, dvt: DvtSecureSocketProxyService):
    ...
    patterns = ["MobileGestalt.plist", "Marking download as [finished]"]
    attempt = 0
    ...
    while attempt < 2 and not found:
        for syslog_entry in OsTraceService(lockdown=service_provider).syslog():
            ...
            fn = posixpath.basename(syslog_entry.filename)
            if (fn in ('bookassetd', 'itunesstored')) and all(p in msg for p in patterns):
                found = True
                break
            if time.time() > deadline:
                click.secho("Timeout. Retrying...", fg="yellow")
                procs = OsTraceService(lockdown=service_provider).get_pid_list().get("Payload")
                pid_bookassetd = next((pid for pid, p in procs.items() if p['ProcessName'] == 'bookassetd'), None)
                pid_books = next((pid for pid, p in procs.items() if p['ProcessName'] == 'Books'), None)
                if pid_bookassetd:
                ...
                break
    if not found:
        raise Exception("MobileGestalt overwrite not detected")
...
async def create_tunnel(udid):
    ...
    if is_windows:
        try:
            ...
            is_admin = False
        if not is_admin:
            print("It seems like you're not running this script as admin, which is required.")
            sys.exit()
        cmd = [sys.executable, "-m", "pymobiledevice3", "lockdown", "start-tunnel", "--script-mode", "--udid", udid]
        tunnel_process = subprocess.Popen(cmd, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    else:
        cmd = f"sudo pymobiledevice3 lockdown start-tunnel --script-mode --udid {udid}"
    ...
    while True:
        ...
        if tunnel_process.poll() is not None:
            ...
            if error:
                lines = [e.decode(errors="ignore") for e in error]
                not_connected = any("connected" in l.lower() for l in lines)
                admin_error = any(("admin" in l.lower()) or ("access is denied" in l.lower()) for l in lines)
                not_found = any(("no module named pymobiledevice3" in l.lower()) or ("not recognized" in l.lower()) for l in lines)
                if not_connected:
                    print("It seems like your device isn't connected.", lines)
                elif admin_error:
                    print("It seems like you're not running this script as admin, which is required.", lines)
                elif not_found:
                    print("It seems like pymobiledevice3 is not installed or not on PATH.", lines)
                else:
                    print("Error opening a tunnel.", lines)
                sys.exit()
            break
    rsd_str = str(rsd_val)
    ...
    try:
        ...
        if device_version >= parse_version('17.0'):
            ...
            if available_address:
                click.secho("Connecting to tunnel...", fg="yellow")
                retries = 5
                for i in range(retries):
                    try:
                        ...
                        click.secho(f"Tunnel connect failed, retry {i+1}/{retries}: {e}", fg="red")
                        time.sleep(1)
                else:
                    raise Exception("Failed to connect to tunnel after retries")
            else:
